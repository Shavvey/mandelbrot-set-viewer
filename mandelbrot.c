...
/* Linearly Maps a given value within a given range to a new value within a new
 * range */
...
int cindex(int x, int y, int width) { return ((y * width) + x); }
...
struct Complex_n *complex_add(struct Complex_n *a, struct Complex_n *b,
                              struct Complex_n_bin *bin) {
...
/*Performs iterations of z_n+1 = z + C  to determine if a given coordinate is
 * within the mandelbrot set */
struct Complex_n_bin *set_iterate(struct Complex_n *a, int iterations,
                                  struct Complex_n_bin *bin) {
  int i = 0;
  // struct Complex_n *z = (struct Complex_n *)malloc(sizeof(struct
  // Complex_n*));
  ...
  for (; i < iterations; i++) {
    ...
    // z = complex_add(complex_square(complex_square(z, bin), bin), a, bin);
    // //Other Func
...
void mandel_update(struct Mandel_Input *man_i, struct Mandel_Data *man_d) {
  ...
  for (int i = 0; i < (man_i->width * man_i->height); i++) {
    ...
    // printf("%d:%d", (man_d->complex_array + i)->rc ,(man_d->complex_array +
    // i)->ic);
  }
  ...
  for (int x = 0; x < (man_i->width); x++) {
    ...
    for (int y = 0; y < (man_i->height); y++) {
      ...
      ((man_d->complex_array)[cindex(x, y, man_i->width)]).rc =
          l_map(x, 0.0f, man_i->width, -1, 1) * man_i->zoomfac + man_i->xoff;
      ((man_d->complex_array)[cindex(x, y, man_i->width)]).ic =
          l_map(y, 0.0f, man_i->height, -1, 1) * man_i->zoomfac + man_i->yoff;

      // Calculates fractal using set_iterate, saves in an extra variable so it
      // can be transferred to the bin_array
      bin_cpy = set_iterate(
          &((man_d->complex_array)[cindex(x, y, man_i->width)]),
          man_i->num_iterations,
          &((man_d->complex_bin_array)[cindex(x, y, man_i->width)]));
...
void *threaded_mandel_update(void *th_args) {
  ...
  for (int i = start_index; i <= end_index; i++) {
    ...
    ((man_d->complex_array)[i]).ic =
        l_map((i / width), 0.0f, man_i->height, -1, 1) * man_i->zoomfac +
        man_i->yoff;

    // Calculates fractal using set_iterate, saves in an extra variable so it
    // can be transferred to the bin_array
    bin_cpy = set_iterate(&((man_d->complex_array)[i]), man_i->num_iterations,
...
void update_mandel_create_threads(struct Thread_Args *th_args_a,
  ...
  for (int i = 0; i < N_THREADS; i++) {
    ...
    printf("\n\nSpawning Thread%d", i);
    pthread_create(&threads[i], NULL, threaded_mandel_update,
                   (void *)&th_args_a[i]);
  }
...
void mandel_draw(uint32_t *pixel_buffer,
                 struct Complex_n_bin *complex_bin_array,
                 struct Color_Info *color_i, struct Mandel_Input *man_i) {
  double normalized_color = 0;
  uint32_t color = 255;
  for (int x = 0; x < (man_i->width); x++) {

    for (int y = 0; y < (man_i->height); y++) {
      // Set Pixel Color According to cindex that the complex number landed on
      // in the loop
      normalized_color = l_map(complex_bin_array[y * man_i->width + x].i, 0,
                               man_i->num_iterations, 0, 1);

      // Assign pixel color
      color =
          color_calc(normalized_color, color_i->red_bias, color_i->green_bias,
                     color_i->blue_bias, color_i->color_coef);
      ...
      // pixel_buffer[(man_i->width * man_i->height)/2 + man_i->width/2] =
      // 0xFFFFFFFF;
    }
  }
}
...
void print_help() {
  ...
  printf("Commands: \n");
  printf("\t1, 2, 3 :Increase (Red, Green, Blue) Channel Color Bias "
         "Respectively\n");
  printf("\tq, w, e :Decrease (Red, Green, Blue) Channel Color Bias "
         "Respectively\n");
  printf("\t4 :Increase Color Coefficient, has the effect of changing the rate "
         "of color change\n");
  ...
  printf("\tENTER :Refresh, re-renders and calculates set with (possibly) "
         "updated parameters\n");
  printf("\tUP_ARROW :Increase number of iterations by 50, does not "
         "automatically re-calculate, user ENTER to refresh\n");
  printf("\tDOWN_ARROW :Decrease number of iterations by 50, does not "
         "automatically update\n");
  printf("\tp :Print Statistical information to console, including location, "
         "zoomfac.etc\n");
  printf("\ts :Saves a screenshot as bmp in ./images/ folder, also saves "
         "location,color,zoom and executable command data as a .met file\n\t   "
         " also saves an executable command with generated parameters to the "
         "end of the file \"cool_commands\"\n");

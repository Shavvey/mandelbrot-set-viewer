...
int main(int argc, char *args[]) {
  ...
  // sure the window is shown when created
  window = SDL_CreateWindow("Mandelbrot Renderer with SDL2",
  ...
  man_d->complex_bin_array = (struct Complex_n_bin *)malloc(
  ...
  */
  ...
  while (!ended) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_MOUSEBUTTONDOWN:
        ...
        case SDL_BUTTON_LEFT:
          man_i->xoff = l_map(event.button.x, 0.0f, SCREEN_WIDTH,
                              -1 * man_i->zoomfac + man_i->xoff,
                              1 * man_i->zoomfac + man_i->xoff);
          man_i->yoff = l_map(event.button.y, 0.0f, SCREEN_HEIGHT,
      ...
      case SDL_KEYUP:
        ...
        if (event.key.keysym.sym == SDLK_s) {
          ...
          SDL_Surface *surface = SDL_CreateRGBSurfaceWithFormat(
              0, SCREEN_WIDTH, SCREEN_HEIGHT, 24, SDL_PIXELFORMAT_RGBA8888);
          ...
          fprintf(metadata, "./mandl %0.17g %d %0.17g %0.17g %d %d %d %d\n",
                  man_i->zoomfac, man_i->num_iterations, man_i->xoff,
                  man_i->yoff, color_i->red_bias, color_i->green_bias,
          ...
          if (cool_commands) {
            ...
            fprintf(cool_commands,
                    "./mandl %0.17g %d %0.17g %0.17g %d %d %d %d\n",
                    man_i->zoomfac, man_i->num_iterations, man_i->xoff,
                    man_i->yoff, color_i->red_bias, color_i->green_bias,
                    color_i->blue_bias, color_i->color_coef);
    ...
    SDL_RenderCopy(renderer, texture, NULL,
                   NULL);        // Copies Texture to the renderer
